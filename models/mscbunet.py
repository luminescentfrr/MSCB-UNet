...
class InceptionDWConv2d(nn.Module):
    def __init__(self, in_channels, square_kernel_size=3, band_kernel_size=11, branch_ratio=0.125):
        ...
        # Calculate the number of channels for the branch convolution layer gc based on branch_ratio
        gc = int(in_channels * branch_ratio)

        """
        kernel_size: Specifies the size of the convolution kernel (or filter). It determines the area size covered by the input tensor during each convolution operation.
                    If it is an integer, it represents a square kernel;
                    If it is in tuple form like (height, width), it specifies the size of the kernel in height and width directions.
        padding    : Adds extra zero values around the edges of the input tensor to control the spatial dimensions of the output tensor. It can avoid loss of boundary information due to convolution operations.
                    If it is an integer, the same number of zeros is added uniformly around each boundary;
                    If it is in tuple form like (top, bottom, left, right), it specifies the padding size for the top, bottom, left, and right directions respectively.
                    For 2D convolution, a common simplified form is (height_padding, width_padding).
        """
        # Depthwise separable convolution layer, processing square areas, using gc groups for convolution
        self.dwconv_hw = nn.Conv2d(gc, gc, square_kernel_size, padding=square_kernel_size // 2, groups=gc)

        # Depthwise separable convolution layer, processing horizontal W direction strip areas, using gc groups for convolution
        self.dwconv_w = nn.Conv2d(gc, gc, kernel_size=(1, band_kernel_size), padding=(0, band_kernel_size // 2), groups=gc)
        # Depthwise separable convolution layer, processing vertical H direction strip areas, using gc groups for convolution
        self.dwconv_h = nn.Conv2d(gc, gc, kernel_size=(band_kernel_size, 1), padding=(band_kernel_size // 2, 0), groups=gc)

        # Calculate the indices used to split the input tensor x to allocate x to different convolution paths
        self.split_indexes = (in_channels - 3 * gc, gc, gc, gc)

    # Forward propagation function, defines how data flows through the network
    def forward(self, x):
        # Use the pre-calculated index split_indexes to split the input tensor x, corresponding to the parts that do not go through convolution (identity ID), square convolution, wide strip convolution, and high strip convolution
        """
        ...
        # Concatenate the tensors processed by different paths along the channel dimension and return as the final output
        return torch.cat(
            (x_id, self.dwconv_hw(x_hw), self.dwconv_w(x_w), self.dwconv_h(x_h)),
            dim=1,
        )
# Identity mapping/square kernel/two orthogonal kernels
...
class MultiScaleChannelAttention(nn.Module):
    def __init__(self, in_channels, ratio=8):
        ...
        # Replace normal convolution with depthwise separable convolution
...
class MultiScalePAM(nn.Module):
    def __init__(self, in_channels, reduction_ratio=8):
        ...
        self.reduction_ratio = reduction_ratio  # Spatial downsampling ratio
        ...
        # Update the input channel number of the convolution layer
        ...
        # Add downsampling and upsampling layers for spatial attention
    ...
    def forward(self, x):
        ...
        # Downsample feature maps to reduce memory usage
        ...
        # Calculate attention (on the downsampled spatial scale)
        attention = self.softmax(torch.bmm(query, key))
        out = torch.bmm(value, attention.permute(0, 2, 1))
        
        # Reshape and upsample back to original size
...
class MPCBAM(nn.Module):
    def __init__(self, in_channels, ratio=8, ablation_config=None):
        ...
        if ablation_config is None:
            ablation_config = {
                ...
                'position_attention': True,
                'feature_fusion': True
            }
        
        # Decide whether to use each component based on the configuration
        if ablation_config['channel_attention']:
            self.ms_channel_attention = MultiScaleChannelAttention(in_channels, ratio)
        if ablation_config['spatial_attention']:
            self.ms_spatial_attention = MultiScaleSpatialAttention()
        if ablation_config['position_attention']:
            self.ms_position_attention = MultiScalePAM(in_channels)
        if ablation_config['feature_fusion']:
    ...
    def forward(self, x):
        ...
        if self.config['channel_attention']:
            x_out = self.ms_channel_attention(x_out)
        
        # Spatial attention
        if self.config['spatial_attention']:
            x_out = self.ms_spatial_attention(x_out)
        
        # Position attention
        if self.config['position_attention']:
            x_p = self.ms_position_attention(x)
            x_out = x_out + x_p if self.config['spatial_attention'] or self.config['channel_attention'] else x_p
...
class CBG(nn.Module):
    def __init__(self, in_c, out_c, kernel_size=3, padding=1, dilation=1, stride=1, act=True):
        super().__init__()
        self.act = act
        # Change normal convolution to depthwise separable convolution
        self.conv = nn.Sequential(
...
# Define convolution dynamic feature attention class
class MS_CDFA(nn.Module):
    def __init__(self, in_c, out_c=128, num_heads=4, kernel_size=3, padding=1, 
                 stride=1, attn_drop=0., proj_drop=0., ablation_config=None):
        super().__init__()
        if ablation_config is None:
            ablation_config = {
        ...
        dim = out_c
        self.dim = dim
        self.num_heads = num_heads if ablation_config['multi_head_attention'] else 1
        ...
        self.attn_fg = nn.Conv2d(dim, kernel_size ** 4 * self.num_heads, kernel_size=1, bias=True)
        self.attn_bg = nn.Conv2d(dim, kernel_size ** 4 * self.num_heads, kernel_size=1, bias=True)
        ...
        # Edge enhancement related components
        if ablation_config['edge_enhancement']:
        ...
        if ablation_config['input_transform']:
            ...
            self.input_conv = nn.Conv2d(in_c, dim, kernel_size=1)
            self.output_conv = nn.Conv2d(dim, dim, kernel_size=1)

    def enhance_edges(self, x):
        if not self.config['edge_enhancement']:
            return x
    ...
    def forward(self, x, fg, bg):
        # Input transformation
        if self.config['input_transform']:
            x = self.input_cbr(x)
        else:
            x = self.input_conv(x)
        ...
        if self.config['input_transform']:
            ...
            out = self.output_conv(x_weighted_bg)
...
class MSCB_UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        d5 = self.up5(bridge)
        d5_cat = torch.cat([d5, e5], dim=1)  # Concatenate features
        d5 = self.dec5(d5_cat)  # Main path processing
        d5 = d5 + self.res_adapter_dec5(d5_cat)  # Add residual
